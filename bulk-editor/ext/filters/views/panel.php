...
$wpbe_form_allowed = array(
	'select' => array('id' => true, 'class' => true, 'name' => true, 'disabled' => true, 'multiple' => true, 'data-placeholder' => true, 'data-field' => true, 'data-post-id' => true, 'size' => true, 'onchange' => true, 'style' => true),
	'option' => array('value' => true, 'selected' => true, 'disabled' => true, 'style' => true),
	'optgroup' => array('label' => true),
	'input' => array('type' => true, 'id' => true, 'class' => true, 'name' => true, 'value' => true, 'placeholder' => true, 'min' => true, 'max' => true, 'data-key' => true, 'data-val-id' => true, 'data-post-id' => true, 'onmouseover' => true, 'disabled' => true),
	'a' => array('href' => true, 'class' => true, 'data-val-id' => true),
				...
				<table class="wpbe-full-width">
					<tr>
						<td class="wpbe-full-width">
							<select class="chosen-select wpbe_filter_select wpbe-full-width" multiple="" id="wpbe_filter_taxonomies_<?php echo esc_attr($t->name) ?>" name="wpbe_filter[taxonomies][<?php echo esc_attr($t->name) ?>][]" data-placeholder="<?php echo esc_attr($t->label) ?>">
								<?php if (!empty($terms)): ?>
	...
	$filter_keys = array(
		...
		'post_title' => array(
			'placeholder' => sprintf(
				...
				esc_html__('[%s] title ...', 'bulk-editor'), $WPBE->settings->current_post_type),
			'behavior_options' => $behavior_options
		),
		'post_content' => array(
			'placeholder' => sprintf(
				/* translators: %s: post type name */
				esc_html__('[%s] content ...', 'bulk-editor'), $WPBE->settings->current_post_type),
			'behavior_options' => $behavior_options
		),
		'post_excerpt' => array(
			'placeholder' => sprintf(
				/* translators: %s: post type name */
				esc_html__('[%s] excerpt ...', 'bulk-editor'), $WPBE->settings->current_post_type),
			'behavior_options' => $behavior_options
		),
		'post_name' => array(
			'placeholder' => sprintf(
				/* translators: %s: post type name */
				esc_html__('[%s] slug ...', 'bulk-editor'), $WPBE->settings->current_post_type),
			'behavior_options' => $behavior_options
		),
		'post_password' => array(
			'placeholder' => sprintf(
				/* translators: %s: post type name */
				esc_html__('[%s] password ...', 'bulk-editor'), $WPBE->settings->current_post_type),
			'behavior_options' => $behavior_options
		),
	);
		...
		<div class='filter-unit-wrap'>
			<div class="col-lg-10">
				<div class="wpbe-pr2px">
					<input type="text" placeholder="<?php echo esc_attr($item['placeholder']) ?>" name="wpbe_filter[<?php echo esc_attr($key) ?>][value]" value="" />
				</div>
			</div>
			<div class="col-lg-2">
		...
		<div class='filter-unit-wrap filter-unit-wrap-numbers'>
			<?php foreach ($filter_keys as $key => $item) : ?>
				...
				<div class="col-lg-6 prmb2">
					<input type="number" name="wpbe_filter[<?php echo esc_attr($key) ?>][from]" min="0" placeholder="<?php echo esc_attr($item['placeholder_from']) ?>" value="" /><br />
				</div>
				<div class="col-lg-6 prmb2">
					<input type="number" name="wpbe_filter[<?php echo esc_attr($key) ?>][to]" min="0" placeholder="<?php echo esc_attr($item['placeholder_to']) ?>" value="" />
				</div>
...
function wpbe_filter_draw_other() {
	...
	<div class='filter-unit-wrap wpbe_filter_draw_other'>
		...
		<div class="col-lg-6 prmb2">
			<div class="pl1">
				<input type="number" name="wpbe_filter[menu_order_from]" min="0" placeholder="<?php esc_attr_e('Menu order from', 'bulk-editor') ?>" value="" /><br />
			</div>
		</div>
		<div class="col-lg-6 prmb2">
			<div class="pr1">
				<input type="number" name="wpbe_filter[menu_order_to]" min="0" placeholder="<?php esc_attr_e('Menu order to', 'bulk-editor') ?>" value="" />
			</div>
		</div>
		...
		<div class="col-lg-6 prmb2">
			<div class="pl1">
				<input type="number" name="wpbe_filter[post_parent_from]" min="0" placeholder="<?php esc_attr_e('Post parent from', 'bulk-editor') ?>" value="" /><br />
			</div>
		</div>
		<div class="col-lg-6 prmb2">
			<div class="pr1">
				<input type="number" name="wpbe_filter[post_parent_to]" min="0" placeholder="<?php esc_attr_e('Post parent to', 'bulk-editor') ?>" value="" />
			</div>
		</div>
		...
		<div class="col-lg-6 prmb2">
			<div class="pl1">
				<?php
				echo wp_kses(WPBE_HELPER::draw_select(array(
						'options' => array(-1 => sprintf(
							/* translators: %s: post type name */
							esc_html__('[%s] status', 'bulk-editor'), $WPBE->settings->current_post_type)) + $fields['post_status']['select_options'],
						'field' => 'post_status',
						'post_id' => 0,
		...
		<?php foreach ($calendar_fields as $ff) : ?>
			...
			<?php if ($fields[$ff]['field_type'] === 'meta'): ?>
				...
				<div class="col-lg-6 prmb2">
					<div class="pl1">
						<?php
						echo wp_kses(WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_from", sprintf(
									/* translators: %s: field title */
									esc_html__('%s from', 'bulk-editor'), $fields[$ff]['title']), $ff . '_from', '', "wpbe_filter[{$ff}][from]", true), $wpbe_form_allowed);
						?>
					</div>
				</div>
				<div class="col-lg-6 prmb2">
					<div class="pr1">
						<?php
						echo wp_kses(WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_to", sprintf(
									/* translators: %s: field title */
									esc_html__('%s to', 'bulk-editor'), $fields[$ff]['title']), $ff . '_to', '', "wpbe_filter[{$ff}][to]", true), $wpbe_form_allowed);
						?>
					</div>
				</div>
			...
			<?php else: ?>
				...
				<div class="col-lg-6 prmb2">
					<div class="pl1">
						...
						echo wp_kses(WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_from", sprintf(
									/* translators: %s: field title */
									esc_html__('%s from', 'bulk-editor'), $fields[$ff]['title']), $ff . '_from', '', "wpbe_filter[{$ff}_from]", true), $wpbe_form_allowed);
						?>
					</div>
				</div>
				<div class="col-lg-6 prmb2">
					<div class="pr1">
						...
						echo wp_kses(WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_to", sprintf(
									/* translators: %s: field title */
									esc_html__('%s to', 'bulk-editor'), $fields[$ff]['title']), $ff . '_to', '', "wpbe_filter[{$ff}_to]", true), $wpbe_form_allowed);
						?>
					</div>
				</div>
