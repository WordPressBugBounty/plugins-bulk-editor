...
final class WPBE_META extends WPBE_EXT {
	...
	public function wpbe_ext_scripts() {
		...
		<script>
			...
			lang.<?php echo esc_attr($this->slug) ?>.enter_key = '<?php esc_html_e('Meta key cannot be empty!', 'bulk-editor') ?>';
			lang.<?php echo esc_attr($this->slug) ?>.enter_prod_id = '<?php esc_html_e('Enter a post ID!', 'bulk-editor') ?>';
			lang.<?php echo esc_attr($this->slug) ?>.no_keys_found = '<?php esc_html_e('No meta keys found!', 'bulk-editor') ?>';
			lang.<?php echo esc_attr($this->slug) ?>.new_key = '<?php esc_html_e('New meta key', 'bulk-editor') ?>';
		</script>
		<?php
	}
			...
			foreach ($metas as $m) {
				if (in_array($m['meta_key'], $this->settings->get_fields_keys())) {
					if ($m['type'] == 'number' AND $m['edit_view'] != 'switcher' AND $m['edit_view'] != 'calendar') {
						$data[$m['meta_key']] = array(
							'placeholder_from' => sprintf(
								/* translators: %s: time */
								esc_html__('%s from', 'bulk-editor'), $m['title']),
							'placeholder_to' => sprintf(
								/* translators: %s: time */
								esc_html__('%s to', 'bulk-editor'), $m['title']),
							//'css_classes' => 'not-for-variations'
							'direct' => TRUE,
							'css_classes' => ''
						);
