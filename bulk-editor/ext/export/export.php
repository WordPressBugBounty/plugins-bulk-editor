...
final class WPBE_EXPORT extends WPBE_EXT {
	...
	public function wpbe_ext_scripts() {
		...
		<script>
			...
			lang.<?php echo esc_attr($this->slug) ?>.want_to_export = '<?php esc_html_e('Should the export be started?', 'bulk-editor') ?>';
			lang.<?php echo esc_attr($this->slug) ?>.exporting = '<?php esc_html_e('Exporting', 'bulk-editor') ?> ...';
			lang.<?php echo esc_attr($this->slug) ?>.exported = '<?php esc_html_e('Exported', 'bulk-editor') ?> ...';
			lang.<?php echo esc_attr($this->slug) ?>.export_is_going = "<?php esc_html_e('ATTENTION: Export operation is going!', 'bulk-editor') ?>";
		</script>
		<?php
	}
	...
	public function wpbe_export_posts_count() {
		...
		$format = isset($_REQUEST['format']) ? sanitize_key($_REQUEST['format']) : '';
	...
	public function wpbe_export_posts() {
		...
		if (isset($_REQUEST['combination'])) {
			$combination = WPBE_HELPER::sanitize_array((array) $_REQUEST['combination']);
		}
