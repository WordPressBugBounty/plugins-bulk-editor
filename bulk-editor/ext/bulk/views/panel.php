...
<div class="notice notice-warning">
    <p>
		<?php
		/* translators: %s: posts selection */
		printf(esc_html__('Bulk editing will be applied to: %s', 'bulk-editor'), '<span class="wpbe_action_will_be_applied_to">' . sprintf(esc_html__('all the [%s] on the site', 'bulk-editor'), esc_html($WPBE->settings->current_post_type)) . '</span>')
		?>
	</p>
</div>
...
<div class="wpbe_delete_wraper wpbe-float-right">
    ...
    <a href="#" disabled='disabled' class="button button-primary button-large" id="wpbe_bulk_delete_posts_btn" ><?php
		/* translators: %s: name */
		printf(esc_html__('Delete [%s]!', 'bulk-editor'), esc_html($WPBE->settings->current_post_type)) ?></a>
...
<ul>
    ...
    <li>* <?php
		printf(
			/* translators: %s: link to backup plugin */
			esc_html__('Time by time (one time per week for example) - make the backup of your site database. For example by %s', 'bulk-editor'), wp_kses(WPBE_HELPER::draw_link(
				array(
					'href' => 'https://wordpress.org/plugins/wp-migrate-db/',
					'title' => esc_html__('this plugin', 'bulk-editor'),
					'target' => '_blank'
				)
			), array('a' => array('href' => true, 'title' => true, 'target' => true, 'class' => true))))
		?><br /></li>
    <li>
				...
				<div class="col-lg-4">
					<div class='filter-unit-wrap <?php echo esc_attr($fields[$field_key]['css_classes']) ?> <?php if (!$fields[$field_key]['direct']): ?>wpbe-direct-field<?php endif; ?>'>
						<div class="col-lg-1">
							...
							<input type="checkbox" title='<?php esc_html_e('select it to use', 'bulk-editor') ?>' name="wpbe_bulk[is][<?php echo esc_attr($field_key) ?>]" class="bulk_checker" data-field-key="<?php echo esc_attr($field_key) ?>" data-title="<?php esc_attr($fields[$field_key]['title']) ?>" value="1" /><br />
							<?php endif; ?>
						</div>
						<div class="col-lg-11">

							<div>
								<?php echo wp_kses_post(WPBE_HELPER::draw_meta_popup_editor_btn($field_key, 0, $fields[$field_key]['title'])); ?>
							</div>
				...
				<div class="col-lg-4">
					<div class='filter-unit-wrap <?php echo esc_attr($field['css_classes']) ?> <?php if (!$fields[$field_key]['direct']): ?>wpbe-direct-field<?php endif; ?>'>
						<div class="col-lg-1">
							...
							<?php if ($fields[$field_key]['direct']): ?>
								<input type="checkbox" title='<?php esc_attr('select it to use', 'bulk-editor') ?>' name="wpbe_bulk[is][<?php echo esc_attr($field_key) ?>]" class="bulk_checker" data-field-key="<?php echo esc_attr($field_key) ?>" data-title="<?php esc_attr($field['title']) ?>" value="1" /><br />
							<?php endif; ?>
						...
						<div class="col-lg-2">
							<select name="wpbe_bulk[<?php echo esc_attr($field_key) ?>][behavior]" disabled="" class="wpbe_bulk_value_signs" data-key="<?php echo esc_attr($field_key) ?>">
...
function wpbe_bulk_draw_nums($filter_keys) {
		...
		<div class="col-lg-4">
			<div class='filter-unit-wrap <?php echo esc_attr($field['css_classes']) ?> <?php if (!$field['direct']): ?>wpbe-direct-field<?php endif; ?>'>
				<div class="col-lg-1">
					<div class="wpbe_height_4"></div>
					<?php if ($field['direct']): ?>
						<input type="checkbox" title='<?php esc_html_e('select it to use', 'bulk-editor') ?>' name="wpbe_bulk[is][<?php echo esc_attr($field_key) ?>]" class="bulk_checker" data-field-key="<?php echo esc_attr($field_key) ?>" data-title="<?php esc_attr($field['title']) ?>" value="1" /><br />
					<?php endif; ?>
				</div>
				<div class="col-lg-7">
	...
	<div class="col-lg-1">
		<?php
		WPBE_HELPER::draw_tooltip(sprintf(
				/* translators: %s: link */
				esc_html__('Select how to round float values fractions in the numeric fields. Works for numeric meta fields. Read more %s.', 'bulk-editor'), WPBE_HELPER::draw_link(array(
				...
				<div class='filter-unit-wrap not-for-variations wpbe-overflow-vis <?php if (!$WPBE->settings->get_fields()[$t->name]['direct']): ?>wpbe-direct-field<?php endif; ?>'>
					...
					<div class="col-lg-1">
						...
						<?php if ($WPBE->settings->get_fields()[$t->name]['direct']): ?>
							<input type="checkbox" title='<?php esc_html_e('select it to use', 'bulk-editor') ?>' name="wpbe_bulk[is][<?php echo esc_attr($t->name) ?>]" class="bulk_checker" data-field-key="<?php echo esc_attr($t->name) ?>" data-title="<?php echo esc_attr($t->label) ?>" value="1" /><br />
						<?php endif; ?>
					</div>

					<div class="col-lg-9">
						<select class="chosen-select wpbe_filter_select" disabled="" multiple="" id="wpbe_bulk_taxonomies_<?php echo esc_attr($t->name) ?>" name="wpbe_bulk[<?php echo esc_attr($t->name) ?>][value][]" data-placeholder="<?php echo esc_attr($t->label) ?>">
							<?php if (!empty($terms)): ?>
		...
		<div class="col-lg-4">
			<div class='filter-unit-wrap <?php echo esc_attr($field['css_classes']) ?> <?php if (!$field['direct']): ?>wpbe-direct-field<?php endif; ?>'>
				...
				<div class="col-lg-11">
					...
					$opt = array(-1 => sprintf(
							/* translators: %s: name */
							esc_html__('Set: %s', 'bulk-editor'), esc_html($field['title'])));
			...
			<div class='filter-unit-wrap <?php echo esc_attr($fields[$current_field_key]['css_classes']) ?> <?php if (!$fields[$current_field_key]['direct']): ?>wpbe-direct-field<?php endif; ?>'>
				<div class="col-lg-1">
					...
					<?php if ($fields[$current_field_key]['direct']): ?>
						<input type="checkbox" title='<?php esc_html_e('select it to use', 'bulk-editor') ?>' name="wpbe_bulk[is][<?php echo esc_attr($current_field_key) ?>]" class="bulk_checker" data-field-key="<?php echo esc_attr($current_field_key) ?>" data-title="<?php esc_attr($fields[$current_field_key]['title']) ?>" value="1" /><br />
					<?php endif; ?>
				</div>
				<div class="col-lg-4">
					...
					<div class='filter-unit-wrap <?php echo esc_attr($fields[$current_field_key]['css_classes']) ?> <?php if (!$fields[$current_field_key]['direct']): ?>wpbe-direct-field<?php endif; ?>'>
						...
						<div class="col-lg-11">
							<div class="content-wrap">
								<?php
								echo wp_kses(WPBE_HELPER::draw_calendar($current_field_key, $fields[$current_field_key]['title'], $current_field_key, '', 'wpbe_bulk[' . $current_field_key . '][value]', true), array(
                                    'input' => array('type' => true, 'onmouseover' => true, 'data-title' => true, 'data-val-id' => true, 'value' => true, 'class' => true, 'placeholder' => true, 'data-key' => true, 'data-post-id' => true, 'id' => true, 'name' => true),
