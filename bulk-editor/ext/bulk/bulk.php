...
final class WPBE_BULK extends WPBE_EXT {
	...
	public function wpbe_ext_scripts() {
		...
		<script>
			...
			lang.<?php echo esc_attr($this->slug) ?>.want_to_bulk = "<?php esc_html_e('Will be edited next:', 'bulk-editor') ?>";
			lang.<?php echo esc_attr($this->slug) ?>.want_to_delete = "<?php		
		printf(
			/* translators: %s: name */
			esc_html__('Sure? Delete [%s]?', 'bulk-editor'), esc_attr($WPBE->settings->current_post_type))
		?>";
			lang.<?php echo esc_attr($this->slug) ?>.deleting = "<?php esc_html_e('Bulk deleting', 'bulk-editor') ?>";
			lang.<?php echo esc_attr($this->slug) ?>.deleted = "<?php		
		printf(
			/* translators: %s: name */
			esc_html__('[%s](s) deleted!', 'bulk-editor'), esc_attr($WPBE->settings->current_post_type))
		?>";
			lang.<?php echo esc_attr($this->slug) ?>.bulking = "<?php esc_html_e('Bulk editing', 'bulk-editor') ?> ...";
			lang.<?php echo esc_attr($this->slug) ?>.bulked = "<?php		
		printf(
			/* translators: %s: name */
			esc_html__('[%s](s) edited! Table redrawing ...', 'bulk-editor'), esc_attr($WPBE->settings->current_post_type))
		?>";
			lang.<?php echo esc_attr($this->slug) ?>.bulked2 = "<?php		
		printf(
			/* translators: %s: name(s) */
			esc_html__('[%s](s) edited!', 'bulk-editor'), esc_attr($WPBE->settings->current_post_type))
		?>";
			lang.<?php echo esc_attr($this->slug) ?>.bulk_is_going = "<?php esc_html_e('ATTENTION: Bulk operation is going!', 'bulk-editor') ?>";
		</script>
		<?php
	}
	...
	public function wpbe_tools_panel_buttons_end() {
		...
		?>
		&nbsp;|&nbsp;<span>
			<?php WPBE_HELPER::draw_advanced_switcher_e(0, 'wpbe_bind_editing', '', array('true' => esc_html__('binded editing', 'bulk-editor'), 'false' => esc_html__('binded editing', 'bulk-editor')), array('true' => 1, 'false' => 0), 'js_check_wpbe_bind_editing', 'wpbe_bind_editing'); ?>
			...
			if ($WPBE->show_notes) {
				...
				if (!empty($fields)) {
					...
					$bind_tooltip = sprintf(
						/* translators: %s: name of fileds */
						esc_html__('In FREE version of the plugin you can change only next fields: %s', 'bulk-editor'), implode(', ', $bind_tooltip));
				}
			}
			?>

			<?php
			$bind_tooltip_text = sprintf(
					/* translators: 1: post type name, 2: post type name */
					esc_html__('In this mode to the all selected [%1$s](s) will be set the value of a [%2$s] field which been edited', 'bulk-editor'), esc_html($WPBE->settings->current_post_type), esc_html($WPBE->settings->current_post_type)) . '. ' . $bind_tooltip;

			WPBE_HELPER::draw_tooltip($bind_tooltip_text);
			?>
		 ...
		 WPBE_HELPER::draw_select_e(array(
			...
			?><?php WPBE_HELPER::draw_tooltip(esc_html__('Select post type you want to manage', 'bulk-editor')) ?>
	...
	public function wpbe_bulk_finish() {
		...
		wp_die(esc_html($this->storage->get_val($count_key) . ''));
	}
